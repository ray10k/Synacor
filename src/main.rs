...
struct Args {
    ...
    #[arg(
        help = "Treat input as raw data.",
        long_help = "Interpret the input as a sequence of 15-bit words, presented in little endian order hexadecimal notation.",
        short = 'r'
    )]
    raw_input: bool,
    #[arg(
        help = "Analyze input, and save result.",
        long_help = "Instead of executing the provided input, analyze the data and save an assembly file at the provided location.",
        short = 'a'
    )]
    ...
    match args.analyze {
        Some(destination) => {
            ...
            if args.raw_input {
                ...
                bytes = File::open(&args.binary_source)
                ...
                if destination.contains(&['/', '\\']) {
                    original_name =
                        &args.binary_source[args.binary_source.find(&['/', '\\']).unwrap()..];
                } else {
        ...
        None => {
            ...
            if args.raw_input {
                ...
                vm = VirtualMachine::init_from_file(&args.binary_source)
                    .expect("Could not parse given file");
            }
...
fn sequence_decypher(input: &str) -> Vec<u16> {
    ...
    (0..words)
        ...
        .map(|start| {
            let left = start * 4;
            let low_byte: u16 =
                u16::from_str_radix(&input[left..left + 2], 16).expect("Malformed sequence input!");
            let high_byte: u16 = u16::from_str_radix(&input[left + 2..left + 4], 16)
                .expect("Malformed sequence input!");
